...
- **Module 2**: Target code(s) and CUI(s) prevalence
...
- **Module 6**: Phenotype label validation

Once **Module 1** (data preparation) is complete, **Modules 2-6** can be run independently, in any order.

---

## -- Requirements --

This pipeline supports the analysis of codified and natural language processing (NLP) features from one or two samples (e.g., cohorts, institutions). You will define a target code(s) (i.e., Phecode) and NLP feature(s) (i.e., CUI) to serve as the focal point of the QC process. Since the goal is quality control (not predictive modeling), we recommend running the pipeline on a random subset of your EHR dataset (e.g., 5,000 patients) to streamline processing and accelerate review.
...
4. (**Module 5** only) A feature similarity dictionary from the ONCE webapp
5. (**Module 6** only) A patient-level labels dataset

Details for each required file type are below.
...
To run **Module 6**, you must provide a patient-level labels dataset. This file contains the phenotype label assigned to each patient and is evaluated against codified and NLP-derived evidence of the target concept. This dataset must contain exactly **one row per patient** and include the following columns:

- `patient_num`: Unique patient identifier (must match the identifier used in codified and NLP datasets)
- `label`: Binary phenotype indicator (0 = negative, 1 = positive)

The label column may represent:

- A rule-based phenotype definition
- A weakly supervised (silver) label
- A manually curated (gold-standard) label
- A model-derived prediction thresholded to 0/1

If probabilistic scores (e.g., values between 0 and 1) are provided instead of binary labels, the pipeline will compute AUC using the continuous scores and apply a classification threshold (default = 0.5) for confusion-matrix metrics. Below is an example of a correctly formatted label dataset:
...
2. Open the R Markdown file and define the following below:
   ...
   - The **target code(s)** (`target_code`) and **target CUI(s)** (`target_cui`) you are analyzing
...
source("Helper_Functions_01132026.R")
...
# Single OR multiple 
  ...
  # Example: multiple
  target_code <- c("PheCode:335", "PheCode:250")
  target_cui  <- c("C0026769", "C2926735")

# Dictionary path
dictionary_path <- "../Data/fake_dict.csv"
...
sample_labels <- list()
  ...
  # Sample 1 (required)
  paths$nlp1 <- "../Data/fake_nlp.csv"
  paths$codified1 <- "../Data/fake_codified.csv"
  sample_labels[["1"]] <- "Site A" 
  
  # Sample 2 (optional)
  paths$nlp2 <- "../Data/fake_nlp_v2.csv"
  paths$codified2 <- "../Data/fake_codified_v2.csv"
...
manual_ONCE_path_code <- "../Data/ONCE_multiple_sclerosis_codified.csv"
manual_ONCE_path_nlp <- "../Data/ONCE_multiple_sclerosis_nlp.csv"

# ----------------------- LABEL QC (MODULE 6 ONLY) -----------------------

# Label file with patient ID and binary label
paths$labels <- "../Data/labels.csv"
...
2. `target_code`: Codified feature(s) of interest (e.g., "PheCode:335")
3. `target_cui`: CUI(s) of interest (e.g., "C0026769")
...
1. `data_inputs`: Cleaned codified and NLP datasets from Module 1
2. `target_code`: Target code(s) of interest (e.g., "PheCode:335")
3. `target_cui`: Target CUI(s) of interest (e.g., "C0026769")
...
This module evaluates the concordance between **patient-level phenotype labels (treated as the reference truth)** and EHR-derived evidence of the target phenotype (treated as predictions). Specifically, structured codified codes and NLP-derived CUIs are used as phenotype indicators and compared against the provided labels.

Three EHR-based prediction definitions are evaluated:

1. **Target Code**: Patient has ≥1 occurrence of the specified `target_code` in the codified dataset
2. **Target CUI**: Patient has ≥1 occurrence of the specified `target_cui` in the NLP dataset
3. **Code + CUI**: Patient has ≥1 occurrence of both the target code *and* the target CUI

For each definition, the module assesses how well EHR-derived phenotype indicators recover the provided labels using standard classification metrics and visual diagnostics. This module is intended as a QC assessment; discrepancies between labels and EHR evidence may reflect:

* Label construction or thresholding errors
* Incomplete code rollups
* NLP extraction gaps
* Temporal misalignment
* Differences in phenotype definition

### *Inputs*

1. `data_inputs`: Cleaned codified and NLP datasets from Module 1 (must include `data_inputs$labels`)
2. `target_code`: Target code(s) of interest (e.g., `"PheCode:335"` or a vector of codes)
3. `target_cui`: Target CUI(s) of interest (e.g., `"C0026769"` or a vector of CUIs)
4. `sample`: Sample identifier used to select datasets (default `"1"`)
5. `threshold`: Classification threshold applied to labels when computing confusion-matrix metrics (default `0.5`)

### *Outputs*

1. `metrics_table`: A summary table reporting, for each EHR-based prediction definition (Code, CUI, Code + CUI):
   * Number of EHR-derived positive and negative phenotypes
   * AUC, sensitivity, specificity, PPV, NPV, and accuracy of the EHR-derived phenotype indicators relative to the truth
   * Confusion matrix counts (TP, TN, FP, FN)
2. `roc_plot`: ROC curve (one per reference definition) showing how well the EHR-derived indicator discriminates between label-positive and label-negative patients
3. `metrics_plot`: Bar plot summarizing sensitivity, specificity, PPV, and NPV for each EHR definition
4. `confusion_plot`: Confusion matrix heatmap summarizing agreement and disagreement between labels (truth) and EHR-derived predictions
5. `eval_dataframe`: Patient-level evaluation dataset containing labels and EHR-based prediction indicators, useful for debugging mismatches

```{r module6}

# Generate module 6 results
results_module6 <- evaluate_labels_module6(
...
print_module6_table(results_module6)

for (ref in names(results_module6$plots)) {
  print(results_module6$plots[[ref]]$roc_plot)
  print(results_module6$plots[[ref]]$metrics_plot)
  print(results_module6$plots[[ref]]$confusion_plot)
}

# Optional: review mismatches
# results_module6$eval_dataframe
