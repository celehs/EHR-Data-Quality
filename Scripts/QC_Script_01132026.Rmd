...
This module evaluates the provided **patient-level labels** against structured and NLP-derived evidence of the target phenotype. Labels are compared to three reference definitions derived from the cleaned datasets in Module 1:
...
For each reference definition, the module quantifies concordance between labels and EHR evidence using standard classification metrics and visual diagnostics. This is intended as a QC check: discrepancies may reflect label construction errors, incomplete code rollups, NLP extraction gaps, or phenotype definition differences.

#### *Inputs*

1. `data_inputs`: Cleaned codified and NLP datasets from Module 1
3. `target_code`: Target code(s) of interest (e.g., `"PheCode:335"`)
4. `target_cui`: Target CUI(s) of interest (e.g., `"C0026769"`)
5. `sample`: Sample identifier used to select datasets (default `"1"`)
6. `threshold`: Classification threshold applied to labels when computing confusion-matrix metrics (default `0.5`)

#### *Outputs*

1. `metrics_table`: A summary table reporting, for each reference definition (Code, CUI, Code + CUI):
   * Number of EHR-defined positives and negatives
   * Confusion matrix counts (TP, TN, FP, FN)
   * AUC, sensitivity, specificity, PPV, NPV, and accuracy of the phenotype labels relative to the EHR-derived reference definition
2. `roc_plot`: ROC curve visualization (one per reference definition) summarizing discriminative performance (AUC)
3. `metrics_plot`: Bar plot summarizing Sensitivity, Specificity, PPV, and NPV (one per reference definition)
4. `confusion_plot`: Confusion matrix heatmap summarizing true/false positives and negatives (one per reference definition)
5. `eval_dataframe`: Patient-level joined dataset used for evaluation (labels + reference indicators), useful for debugging mismatches
